...
The demo application we will be testing consists of two microservices and a Caching server that will all be deployed as part of the integration
...
`resources` goal of FMP of the microservices modules.
...
<2> Using FMP's link:https://maven.fabric8.io/#profiles-predefined[aggregate] profile make's it search for OpenShift resource files inside
...
However the `tests` itself has not code dependencies on the other modules therefore is cleaner to add the dependencies to FMP where they
are actually needed
====
...
With that caching file in place, the only thing we need to do to make Arquillian use it as part of the environment setup is to add:
